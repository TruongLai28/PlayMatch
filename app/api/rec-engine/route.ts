...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const genre of genres) {
      ...
      if (igdbGames?.length) {
        ...
        console.log(
          ...
          JSON.stringify(igdbGames.map((g: any) => ({ id: g.id, name: g.name, total_rating: g.total_rating, cover: g.cover })), null, 2)
        )
      }
    }
    ...
    console.log(
      ...
      JSON.stringify(igdbResultsFinal.map(g => ({ id: g.id, name: g.name, total_rating: g.total_rating, cover: g.cover })), null, 2)
    )

    // Add placeholder cover for DB games that don't have cover data
    const dbPoolWithCovers = dbResultsFinal.map(game => ({
      ...game,
      cover: game.cover || null  // Set to null if no cover, GameCard will handle placeholder
    }))

    // Ensure IGDB games have proper cover structure
    const igdbPoolWithCovers = igdbResultsFinal.map(game => ({
      ...game,
    ...
    return NextResponse.json({
      seed: seedGame,
      dbPool: dbPoolWithCovers,
      igdbPool: igdbPoolWithCovers
    })
  } catch (err) {
